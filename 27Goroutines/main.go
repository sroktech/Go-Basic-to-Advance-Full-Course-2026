...
// Use sync.WaitGroup (lesson 30) or channels (lesson 28) to wait for them.
...
// (channels are covered in depth in lesson 28 — shown briefly here)
...
func main() {
	...
	//   - via channel signal
	//   - via context cancellation (lesson 31)
	//   - via a timeout

	fmt.Println("\n=== 6. Goroutine with timeout ===")
	// select is covered in depth in lesson 29 — this is a preview of its
	// most common use: racing a real result against a timeout.
	done := make(chan bool)
	go func() {
