...
func main() {
	...
	// ─── Nested structs (composition via a named field) ──────────────────────

	// Employee HAS an Address — this is composition, not embedding, because
	// the field has an explicit name (Address). You must write emp.Address.City.
	type Address struct {
		City    string
		Country string
	}
	...
	// This is Go's actual "embedding": list the type with no field name.
	// The field name becomes the type name itself (Address), and its fields
	// and methods are PROMOTED — accessible directly on the outer struct,
	// without an extra ".Address" step. This is how Go shares behavior
	// between types without classical inheritance.
	type Manager struct {
		Name    string
		Age     int
	...
	// Promoted field access — City is reached directly, no ".Address" needed.
	// (mgr.Address.City still works too; both spellings are valid.)
	fmt.Println("Manager:", mgr.Name, "lives in", mgr.City)
}
